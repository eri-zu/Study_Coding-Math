...
document.body.addEventListener('mousemove', (e) => {
  rotationSpeed = (e.clientX - width / 2) * 0.00005; // 0.05前後の値が出力される 
  // / 中心から遠いほど大きな値が出力されるのでカルーセルの回転スピードは早くなる
});
...
for (let i = 0; i < num; i++) {
  ...
  const card = {
    x: Math.cos(angle + baseAngle) * radius, // 円形に配置する
    y: 0,
    z: centerZ + Math.sin(angle + baseAngle) * radius, // 円形に配置する
    img: document.createElement("img"),
    angle: Math.PI * 2 / num * i
  };

  card.img.src = `img/postcard${i}.jpg`;
...
function render() {
  ...
  // zsort関数を元にcardsを並び替え（z座標）
  // canvasでは、デフォルトで、要素を配置した順番で上に表示されていく
  cards.sort(zsort);
  ...
  // shapeを描く
  for (let i = 0; i < num; i++) {
    const perspective = fl / (fl + cards[i].z);

    // cardを描写
    context.save();
    context.scale(perspective, perspective); // cardの大きさを視野角を考慮したものに変換
    context.translate(cards[i].x, cards[i].y); //cardの座標を視野角を考慮したものに変換
    
    context.translate(-cards[i].img.width / 2 , -cards[i].img.height / 2);
    context.drawImage(cards[i].img, 0, 0);
...
function zsort(cardA, cardB) {
  ...
  // cardAのz座標とcardBのz座標を比べる
  // cardAの方が大きければ、戻り値はマイナスになる
  // cardAはcardBより配列の中で前にソートされる  [ cardA, cardB ,.....] みたいな
  // canvasでは、index番号が前の方がz-indexは小さいので、cardAよりcardBが手前にくる
  // ちなみに、z-indexが小さいほど手前
