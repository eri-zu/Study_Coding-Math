...
for (let i = 0; i < num; i++) {
  shapes[i] = {
    ...
    char: String.fromCharCode(utils.randomRange(65, 90)) // A-zを取り出す
  };
}

context.translate(width / 2, height / 2); // windowの中心を(0, 0とする)
context.font = "200px Arial" // fontサイズを指定
...
function render() {
  ...
  for (let i = 0; i < num; i++) {
    ...
    context.fillText(shapes[i].char, 0, 0); // shapeの大きさ(x, y, w, h) とりあえず中心に配置
    context.restore();
