...
for (let i = 0; i < num; i++) {
  ...
  // cards[i].img.src = `postcard${index}.jpg`;
  ...
  card.img.src = `img/postcard${i % 7}.jpg`;
...
function render() {
  ...
  for (let i = 0; i < num; i++) {
    ...
    context.save();
    context.translate(cards[i].x * perspective, cards[i].y * perspective); //cardの座標を視野角を考慮したものに変換
